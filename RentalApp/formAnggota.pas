...
begin
  ...
  else if DataModuleMySQL.zQueryAnggota.Locate('id', txtKode.Text, []) then
  begin
    beep;
    MessageDlg('ID Anggota sudah terdaftar', TMsgDlgType.mtInformation,
      [TMsgDlgBtn.mbOK], 0);
    txtNama.Text := DataModuleMySQL.zQueryAnggotanama.AsString;
    txtPekerjaan.Text := DataModuleMySQL.zQueryAnggotanama.AsString;
    txtAlamat.Text := DataModuleMySQL.zQueryAnggotaalamat.AsString;
    txtHP.Text := DataModuleMySQL.zQueryAnggotatelp.AsString;
  end
  else
  begin
