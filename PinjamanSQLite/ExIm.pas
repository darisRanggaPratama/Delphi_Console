...
begin
  ...
  try
    ...
    // Buka file CSV untuk penulisan
    AssignFile(csvFile, TPath.Combine('upload',
      ExtractFilePath(Application.ExeName) + 'export\angsur_exportperid.csv'));
    Rewrite(csvFile);
    ...
    txtIDPinjam.Clear;
    ShowMessage('Ekspor data berhasil! File: angsur_exportperid.csv');
    Self.Close;
    formHome.pageHome.ActivePage := formHome.tabData;
  finally
    CloseFile(csvFile);
...
begin
  ...
  try
    ...
    // Buka file CSV untuk penulisan
    AssignFile(csvFile, TPath.Combine('upload',
      ExtractFilePath(Application.ExeName) + 'export\angsur_exportperperiod.csv'));
    Rewrite(csvFile);
    ...
    txtPeriod.Clear;
    ShowMessage('Ekspor data berhasil! File: angsur_exportperperiod.csv');
    Self.Close;
    formHome.pageHome.ActivePage := formHome.tabData;
  finally
    CloseFile(csvFile);
